...
Architecture:
  ...
  P1  LOCALIZE: Claude vision per-cloud bboxes
...
Key fixes over prior approaches:
  • Per-crop processing (inside Claude bbox) prevents sheet-level contour welding
...
Usage:
  ...
  python cloud_detector_v2.py input_drawing.jpg --api-key sk-ant-...
"""
...
CLAUDE_MODEL          = "claude-opus-4-8"   # vision model used for localization
CLAUDE_MAX_TOKENS     = 4096                # response budget for the bbox JSON
CLAUDE_MAX_SCALE      = 1568          # max longest side when sending to Claude (vision is downsampled past ~1568px)
SNAP_PAD              = 60            # px to pad around Claude bbox before crop
...
_CLAUDE_PROMPT = """You are analyzing a P&ID (Piping and Instrumentation Diagram) engineering drawing.
...
def _extract_json_array(text: str) -> List[dict]:
    """Parse a JSON array from Claude's text, tolerating markdown fences/prose."""
    text = text.strip()
    if text.startswith("```"):
    ...
    try:
        ...
        if lo != -1 and hi != -1 and hi > lo:
            try:
                ...
                pass
    log.error("Could not parse JSON from Claude response: %s", text[:300])
    return []


def _claude_call(img_bgr: np.ndarray, api_key: str) -> List[dict]:
    """Send image to the Claude vision API, return list of bbox dicts."""
    import base64
...
def locate_with_claude(img_bgr: np.ndarray, api_key: str) -> List[dict]:
    """
    Scale image to max CLAUDE_MAX_SCALE, call Claude, map coords back to orig.
    Returns list of {x0, y0, x1, y1, confidence} in original pixel coords.
    """
    H, W = img_bgr.shape[:2]
    scale = min(1.0, CLAUDE_MAX_SCALE / max(H, W))
    if scale < 1.0:
        send_img = cv2.resize(img_bgr, (int(W * scale), int(H * scale)))
    else:
        send_img = img_bgr

    raw = _claude_call(send_img, api_key)
    if not raw:
        return []
    ...
    for item in raw:
        try:
            ...
            continue
    log.info("Claude returned %d candidate bboxes", len(results))
    return results
...
def stage1_detect(img_bgr: np.ndarray) -> List[np.ndarray]:
    ...
    This catches clouds Claude localization misses. Returns list of contours
...
def detect_all(img_path: str, api_key: str,
    ...
    Requires a Claude API key for localization.
    """
    if not api_key:
        raise ValueError("An Anthropic API key is required (set ANTHROPIC_API_KEY or pass --api-key)")
    ...
    # [P1] Localize with Claude vision
    try:
        bboxes = locate_with_claude(img_bgr, api_key)
    except Exception as e:
        log.error("Claude localization error: %s", e)
        bboxes = []

    if not bboxes:
        log.warning("Claude returned no bounding boxes; "
                    "stage-1 OpenCV detector will provide the only coverage")
...
def main():
    ...
    if not api_key:
        log.error("No Anthropic API key found. Set ANTHROPIC_API_KEY or pass --api-key.")
        sys.exit(1)
