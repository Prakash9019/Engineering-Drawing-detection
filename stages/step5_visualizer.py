...
def draw_candidate(
    ...
    """
    Draw a clean minimal bounding box + small tag label.

    Design principles (fixed from v1):
      - Thin border (2px at full res, scaled by image width)
      - Tag text only — no category prefix, no confidence numbers
      - Tiny filled colour pill as label background (not dark overlay)
      - No dashed inner tag_bbox (too cluttered)
      - Label sits INSIDE the top of the box, not above it
        (avoids overflowing into adjacent symbols)
    """
    # Priority: draw around the TAG TEXT (tag_bbox), not the symbol bubble
    # tag_bbox = where the actual text "FIT-1001" sits on the drawing
    # symbol_bbox = where the instrument circle/diamond sits (offset from text)
    # User requirement: the box MUST be ON the tag text, not above/below it
    coords = _safe_bbox(cand.get("tag_bbox") or {})
    if not coords:
    ...
    # ── Scale thickness and font relative to image size ───────────────────────
    # At 9934px wide: box_thick=2, label font ~0.55
    # At 4000px wide: box_thick=2, label font ~0.55  (consistent)
    scale        = W_img / 8000          # normalise to 8000px reference
    box_thick    = max(2, int(scale * 2))
    lbl_scale    = max(0.4, scale * 0.55)
    lbl_thick    = 1

    # ── Rectangle — clean thin border ─────────────────────────────────────────
    cv2.rectangle(img, (x1, y1), (x2, y2), colour, box_thick)

    # ── Tag label ─────────────────────────────────────────────────────────────
    tag = str(cand.get("tag_text") or "").strip()
    if not tag:
        return   # no label if no tag text — box still drawn

    font = cv2.FONT_HERSHEY_SIMPLEX
    (lw, lh), baseline = cv2.getTextSize(tag, font, lbl_scale, lbl_thick)

    # Position: top-left corner of the box, inside by 2px
    pad = max(2, int(scale * 3))
    lx  = x1 + pad
    ly  = y1 + lh + pad

    # Clamp so label doesn't go off the image
    if ly + baseline > H_img:
        ly = max(lh + pad, y1 - pad)
    if lx + lw > W_img:
        lx = max(0, W_img - lw - pad)

    # ── Filled colour pill background (small, tight) ──────────────────────────
    bg_x0 = lx - pad
    bg_y0 = ly - lh - pad
    bg_x1 = lx + lw + pad
    bg_y1 = ly + baseline + 1

    # Clamp background rect
    bg_x0 = max(0, bg_x0);  bg_y0 = max(0, bg_y0)
    bg_x1 = min(W_img, bg_x1); bg_y1 = min(H_img, bg_y1)

    # Draw filled rectangle in the box colour (same as border)
    cv2.rectangle(img, (bg_x0, bg_y0), (bg_x1, bg_y1), colour, -1)

    # ── Tag text — WHITE on coloured background ───────────────────────────────
    cv2.putText(img, tag, (lx, ly),
                font, lbl_scale, (255, 255, 255), lbl_thick, cv2.LINE_AA)
...
def viz_all_candidates(
    ...
    # Scale relative to 8000px reference — keeps labels readable at any res
    img_scale  = W / 8000
    font_scale = max(0.4, img_scale * 0.55)   # small, crisp labels
    thickness  = max(2, int(img_scale * 2))    # thin clean borders
    ...
    draw_legend(canvas, legend_entries, font_scale=max(0.35, font_scale))

    stats = {"Total": len(candidates), **{k.upper()[:4]: v
             for k, v in cat_counts.items()}}
    draw_stats_banner(canvas, stats, font_scale=max(0.35, font_scale))
...
def viz_duplicates(
    ...
    H, W   = canvas.shape[:2]
    img_scale  = W / 8000
    font_scale = max(0.4, img_scale * 0.55)
    thickness  = max(2, int(img_scale * 2))
    ...
    for cand in deduped:
        ...
        if status == "DISCARDED":
            ...
            if merged_into and merged_into in id_to_cand:
                ...
                sb_d = _safe_bbox(cand.get("tag_bbox") or cand.get("symbol_bbox") or {})
                sb_p = _safe_bbox(primary_c.get("tag_bbox") or primary_c.get("symbol_bbox") or {})
                if sb_d and sb_p:
