...
def detect_pipes_and_lines(img_bgr: np.ndarray) -> list[dict]:
    ...
    _extract_lines(horiz, "horizontal_pipe")
    _extract_lines(vert,  "vertical_pipe")

    # Diagonal / leader lines using Probabilistic Hough
    try:
        ...
        if hough_lines is not None:
            for line in hough_lines:
                ...
                if abs(abs(angle) - 90) < 10 or abs(angle) < 10:
                    continue   # skip H/V already captured
                lines_out.append({
    ...
    log.info("Detected %d lines (%d pipes + leader lines)", len(lines_out),
             sum(1 for l in lines_out if "pipe" in l["type"]))
    return lines_out
