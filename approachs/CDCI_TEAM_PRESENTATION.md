...
## What Needs Improving — Next Steps

**The cloud detection was not active in this run.** The `step2b_cloud_detection.py` script exists and works but was not called between Step 2 and Step 5A. As a result, Step 5A ran without knowing where the cloud boundaries were and extracted tags from the whole drawing instead of just inside the clouds. Running Step 2B will reduce the candidate count to only the in-scope changed items and will significantly improve precision.

**The confidence thresholds need tuning.** Zero tags reached AUTO_ACCEPT because the asset register only has 46 tags but we extracted 233. The register confidence score (C_register) drops to 0.5 for every tag not in the register, which drags the final score below the 0.85 threshold. The formula weights need to be adjusted for projects where the register is incomplete at extraction time.

**The Tag List table was not on this drawing.** The Tag List (the grid of slot numbers and instrument tag columns) lives on the companion drawing, sheet 002 or a separate document. Step 6 correctly identified and extracted the revision history table. The tag list needs to be found on the correct source document.
